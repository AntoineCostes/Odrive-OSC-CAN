...
class ODriveCANManager:
    def __init__(self,
        ...
        self.latest_encoder_flags = { node_id: False for node_id in self.node_ids }
    ...
    def getNewEstimates(self):
        result = {node_id:self.latest_encoder[node_id]
                    if self.latest_encoder_flags[node_id]
                    else None 
                    for node_id in self.node_ids}
        self.latest_encoder_flags = { node_id: False for node_id in self.node_ids }
        return result
            ...
            try:
                ...
                if command_id == ID_GET_ENCODER_ESTIMATES:
                    ...
                    with self.lock:
                        ...
                        self.latest_encoder_flags[node_id] = True
        ...
        with self.lock:
            for node_id in sorted(self.intervals):
                ...
                for command_id in sorted(self.intervals[node_id]):
                    intervals = self.intervals[node_id][command_id]
                    count = len(intervals)
                    ...
                    if len(intervals) == 0:
                        ...
                        command_msg.append(f"0x{command_id:02X}:" f"{count}msg " f"{frequency:.1f}Hz " f"{mean_period*1000:0.1f}ms " f"({min_period*1000:0.0f}" f"-{max_period*1000:0.0f}ms)")
